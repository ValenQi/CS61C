...
inst_needs_relocation:
    srl $t0, $a0, 26
    beq $t0, 2, needs_relocation
    beq $t0, 3, needs_relocation
    li $v0, 0
    jr $ra
needs_relocation:
    li $v0, 1
...
#------------------------------------------------------------------------------
relocate_inst:
    addiu $sp, $sp, -20    # Begin relocate_inst
    sw $ra, 16($sp)
    sw $s3, 12($sp)
    sw $s2, 8($sp)
    sw $s1, 4($sp)
    sw $s0, 0($sp)
    move $s0, $a0    # $s0 = original instr
    move $s1, $a1    # $s1 = byte offset
    move $s2, $a2    # $s2 = symbol table
    move $s3, $a3    # $s3 = relocation table

    move $a0, $s3    # Check if addr is valid
    move $a1, $s1
    jal symbol_for_addr
    beq $v0, $0, relocate_error

    move $a0, $s2    # Search for the corresponding abs addr
    move $a1, $v0
    jal addr_for_symbol
    beq $v0, -1, relocate_error
